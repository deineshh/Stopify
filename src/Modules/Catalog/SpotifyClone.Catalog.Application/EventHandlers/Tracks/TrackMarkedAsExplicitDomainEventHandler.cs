...
internal sealed class TrackMarkedAsExplicitDomainEventHandler(
    ICatalogUnitOfWork unit)
    : INotificationHandler<TrackMarkedAsExplicitDomainEvent>
{
    private readonly ICatalogUnitOfWork _unit = unit;

    public async Task Handle(
        TrackMarkedAsExplicitDomainEvent notification,
        CancellationToken cancellationToken)
    {
        var integrationEvent = new TrackExplicityChangedIntegrationEvent(
                notification.Id.Value, true);
